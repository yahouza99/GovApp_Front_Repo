...
import SchollarshipForm from './schollarshipForm';
import {
  mockSchollarshipRequests,
  requestStatuses,
  getStatusLabel,
  getCitizenName,
  requestTypes,
...
const SchollarshipTable = () => {
  const [items, setItems] = useState(mockSchollarshipRequests);
  ...
  const handleSort = (field) => setSortField(prev => (prev === field ? prev : field));
  ...
  // Edit/Add modes
  if (isEditing && editItem) return <SchollarshipForm initial={editItem} onSave={handleSaveEdit} onCancel={handleCancelEdit} />;
  if (isAdding) return <SchollarshipForm initial={null} onSave={handleSaveAdd} onCancel={handleCancelAdd} />;
    ...
    <div className="h-full flex flex-col bg-white rounded-lg shadow-sm">
      ...
      <div className="p-2 flex-shrink-0">
        <div className="flex items-center justify-between mb-1">
          <div>
            <p className="text-gray-600">Liste et gestion des demandes de bourse</p>
          </div>
        ...
        <div className="flex flex-col gap-3">
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-2">
              <div className="text-sm text-gray-600">{filteredAndSorted.length} demande(s)</div>
              <button
          ...
          <div className="flex flex-wrap items-center gap-3">
            <div className="flex items-center gap-2">
              ...
              <select value={typeFilter} onChange={(e) => setTypeFilter(e.target.value)} className="border border-gray-300 rounded-md px-3 py-1 text-sm focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-green-500">
                ...
                {requestTypes.map(t => (<option key={t.list_id} value={t.list_id}>{t.label}</option>))}
              </select>
            </div>
            <div className="flex items-center gap-2">
              ...
              <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="border border-gray-300 rounded-md px-3 py-1 text-sm focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-green-500">
                <option value="">Tous</option>
                {requestStatuses.map(s => (<option key={s.list_id} value={s.list_id}>{s.label}</option>))}
              </select>
            </div>
            <div className="flex items-center gap-2">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {filteredAndSorted.map((r) => (
                <tr key={r.request_id} onClick={() => handleRowClick(r)} className={`cursor-pointer transition-colors ${selected?.request_id === r.request_id ? 'bg-green-50 border-l-4 border-green-500' : 'hover:bg-gray-50'}`}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{getRequestTypeLabel(r.request_type_id)}</td>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                    <div className="flex items-center gap-2">
                      ...
                      <button onClick={(e) => { e.stopPropagation(); if (window.confirm(`Supprimer la demande #${r.request_id} ?`)) { setItems((prev) => prev.filter(i => i.request_id !== r.request_id)); if (selected?.request_id === r.request_id) setSelected(null); alert('Demande supprimée'); } }} className="px-3 py-1 bg-red-600 text-white rounded-md hover:bg-red-700 transition-colors text-xs" title="Supprimer">Supprimer</button>
                    </div>
                  </td>
                </tr>
              ))}
...
export default SchollarshipTable;
