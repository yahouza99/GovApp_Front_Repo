...
import {
  requestTypes,
  requestStatuses,
  requestTypeExtraColumns,
  govUsers,
  citizens,
  defaultOrgId,
  getRequestTypeLabel,
} from './schollarshipDDL';

const SchollarshipForm = ({ initial, onCancel, onSave }) => {
  const [form, setForm] = useState(() => ({
    request_id: initial?.request_id || null,
    citizen_id: initial?.citizen_id || '',
    org_id: initial?.org_id || defaultOrgId,
    request_type_id: initial?.request_type_id || 901, // Activation par défaut
    request_title: initial?.request_title || 'Demande de bourse',
  ...
  // Recherche pour filtrer/auto-compléter le citoyen
  const [citizenQuery, setCitizenQuery] = useState('');
  const [showCitizenList, setShowCitizenList] = useState(false);

  // Synchroniser le champ avec le citoyen sélectionné (édition)
  useEffect(() => {
  ...
  const dynamicFields = requestTypeExtraColumns[Number(form.request_type_id)]?.extraColumns || [];
    ...
    <div className="h-full flex flex-col bg-white rounded-lg shadow-sm">
      ...
      <div className="p-4 border-b border-gray-200 flex-shrink-0">
        <div className="flex justify-between items-center">
          <div>
            <h2 className="text-2xl font-bold text-gray-900">{form.request_id ? 'Éditer la demande' : 'Nouvelle demande'}</h2>
          </div>
          <div className="space-x-2">
            <button type="button" onClick={onCancel} className="px-4 py-2 border rounded-md">Annuler</button>
            <button type="submit" form="schollarship-form" className="px-4 py-2 bg-green-600 text-white rounded-md">Enregistrer</button>
      ...
      <div className="flex-1 overflow-y-auto p-6">
        <form id="schollarship-form" onSubmit={handleSubmit} className="space-y-6">
          ...
          {/* Données spécifiques (extra_data) */}
          <div className="bg-green-50 p-4 rounded-lg border border-green-200">
            <h3 className="text-lg font-semibold text-green-900 mb-2">
              Informations — {getRequestTypeLabel(form.request_type_id)}
            </h3>
              ...
              <div>
                ...
                {!form.file_1 ? (
                  <label className="px-3 py-2 border rounded-md cursor-pointer hover:bg-gray-50 inline-block">
                    ...
                    <input
                      type="file"
                      onChange={(e) => handleFile('file_1', e.target.files?.[0])}
                      className="hidden"
                    />
                  </label>
                ) : (
                  <div className="flex items-center space-x-3">
                    <a
                      href={`data:application/octet-stream;base64,${form.file_1}`}
                      target="_blank" rel="noopener noreferrer"
                      className="text-blue-600 hover:text-blue-800 underline"
                    >
                      Ouvrir fichier_1.pdf
                    </a>
                    <a
                      href={`data:application/pdf;base64,${form.file_1}`}
                      download={`fichier_1.pdf`}
              ...
              <div>
                ...
                {!form.file_2 ? (
                  <label className="px-3 py-2 border rounded-md cursor-pointer hover:bg-gray-50 inline-block">
                    ...
                    <input
                      type="file"
                      onChange={(e) => handleFile('file_2', e.target.files?.[0])}
                      className="hidden"
                    />
                  </label>
                ) : (
                  <div className="flex items-center space-x-3">
                    <a
                      href={`data:application/octet-stream;base64,${form.file_2}`}
                      target="_blank" rel="noopener noreferrer"
                      className="text-blue-600 hover:text-blue-800 underline"
                    >
                      Ouvrir fichier_2.pdf
                    </a>
                    <a
                      href={`data:application/pdf;base64,${form.file_2}`}
                      download={`fichier_2.pdf`}
                      className="px-3 py-1 border rounded-md hover:bg-gray-50"
                    >
                      Télécharger
                    </a>
              ...
              <div>
                ...
                {!form.file_3 ? (
                  <label className="px-3 py-2 border rounded-md cursor-pointer hover:bg-gray-50 inline-block">
                    ...
                    <input
                      type="file"
                      onChange={(e) => handleFile('file_3', e.target.files?.[0])}
                      className="hidden"
                    />
                  </label>
                ) : (
                  <div className="flex items-center space-x-3">
                    <a
                      href={`data:application/octet-stream;base64,${form.file_3}`}
                      target="_blank" rel="noopener noreferrer"
                      className="text-blue-600 hover:text-blue-800 underline"
                    >
                      Ouvrir fichier_3.pdf
                    </a>
                    <a
                      href={`data:application/pdf;base64,${form.file_3}`}
                      download={`fichier_3.pdf`}
                      className="px-3 py-1 border rounded-md hover:bg-gray-50"
                    >
                      Télécharger
                    </a>
...
export default SchollarshipForm;
